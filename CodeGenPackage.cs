...
public sealed class CodeGenPackage : AsyncPackage
{
    ...
    public static string WEB_PROJECT = ".Web";
    ...
    public static string ApplicationRootNs = "";
    ...
    private void Execute(object sender, EventArgs e)
    {
        ...
        var projectPrefix = target.Project?.Name.Substring(0, target.Project.Name.IndexOf(".Application"));

        NewItemTarget domain = NewItemTarget.Create(_dte, projectPrefix + DOMAIN_PROJECT);
        NewItemTarget infrastructure = NewItemTarget.Create(_dte, projectPrefix + INFRASTRUCTURE_PROJECT);
        NewItemTarget application = NewItemTarget.Create(_dte, projectPrefix + APPLICATION_PROJECT);
        NewItemTarget web = NewItemTarget.Create(_dte, projectPrefix + WEB_PROJECT);

        CoreRootNs = domain.Project.GetRootNamespace();
        InfrastructureRootNs = infrastructure.Project.GetRootNamespace();
        WebRootNs = web.Project.GetRootNamespace();
        ApplicationRootNs = application.Project.GetRootNamespace();

        //var includes = new string[] { "IEntity", "BaseEntity", "AuditableEntityBase" };
        var includes = new string[] { "AuditableEntityBase" };
        ...
        if (target == null && target.Project?.Name == projectPrefix + DOMAIN_PROJECT)
        {
            MessageBox.Show(
        ...
        foreach (string inputName in parsedInputs)
        {
            try
            {
                ...
                var fullName = objectList.Distinct().Where(d => d.FullName.EndsWith(name)).FirstOrDefault().FullName;
                var splittedArray = fullName.Split('.');
                var indexOfEntity = Array.IndexOf(splittedArray, name);
                var moduleName = splittedArray[indexOfEntity - 1];
                ...
                {
                    $"Interfaces/Repositories/{moduleName}/I{name}Repository.cs",
                    $"Features/{moduleName}/{nameofPlural}/Commands/Create{name}Command.cs",
                    $"Features/{moduleName}/{nameofPlural}/Commands/Update{name}Command.cs",
                    $"Features/{moduleName}/{nameofPlural}/Commands/Delete{name}Command.cs",
                    $"Features/{moduleName}/{nameofPlural}/Queries/{name}Response.cs",
                    $"Features/{moduleName}/{nameofPlural}/Queries/GetAll{nameofPlural}Query.cs",
                    $"Features/{moduleName}/{nameofPlural}/Queries/GetById{nameofPlural}Query.cs",
                    $"Mappings/{moduleName}/{name}Profile.cs",
                };
                ...
                // For  infrastructure
                var repo = $"Repositories/{moduleName}/{name}Implement.cs";

                AddItemAsync(objectClass, repo, name, infrastructure).Forget();
                ...
                //{
                //    AddItemAsync(objectClass, item, name, web).Forget();
                //}
            }
    ...
    public static string RemoveFolderNameFromFile(string file, int tailSlashRemove = 1)
    {
        ...
        bool IsImplement = file.Contains("Implement");

        if (!IsImplement)
        {
            return file;
        }
        //if (file.Contains("Implement"))
        //{
        //    folderName = "Infrastructure/Repositories";
        //}
        if (file.Contains("Repositories") && file.Contains("Implement"))
        {
